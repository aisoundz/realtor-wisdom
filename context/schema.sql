...
-- Organizations: RLS disabled. The sensitive data lives in deals/capital_sources/etc.
-- which are scoped via profiles.org_id, so the org table itself doesn't need row-level
-- protection. Keeping it open avoids friction with the auth-via-cookies edge case in
